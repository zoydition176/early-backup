...
var spProductsQuantity = function ($) {
    var SELECTOR ={
        ADD:"[data-action='add']",
        REDUCE:"[data-action='reduce']"
    };
    $(document).on('click', SELECTOR.ADD ,function () {
        var _this = $(this);
        var el = _this.parent('.sp-num-btn-group').siblings('[name="productsQuantity"]');
        var quantity = el.val();
        if(!isNaN(quantity)){
            if(quantity<=99999){
                quantity++;
    ...
    $(document).on('click', SELECTOR.REDUCE ,function () {
        var _this = $(this);
        var el = _this.parent('.sp-num-btn-group').siblings('[name="productsQuantity"]');
        var quantity = el.val();
        if(!isNaN(quantity)){
